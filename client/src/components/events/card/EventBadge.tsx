...
type EventBadgeWrapperProps = PropsWithChildren<{
  className?: string;
}>;
const EventBadgeWrapper = ({ children, className }: EventBadgeWrapperProps) => {
  return (
    <Badge
      className={cn(
        "absolute top-2 right-2 flex items-center gap-1 border-2 py-1 text-sm",
        className,
      )}
    >
      {children}
...
const EventBadge = ({ going }: EventBadgeProps) => {
